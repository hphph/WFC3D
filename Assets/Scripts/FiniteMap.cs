...
public class FiniteMap: MonoBehaviour
{
    ...
    public void InitNewMap()
    {
        ...
        entropySortedQueue = new PriorityQueueSet<ModuleSocket>();
    ...
    {
        Queue<(WFCTools.DirectionIndex, Vector3Int, ModuleSocket)> updateQueue;
        updateQueue = new(WFCTools.NeighboursToSocket(collapsedSocket));
        while(updateQueue.Count > 0)
        {
            ...
            if(isWrapping)
            {
                ...
                queueElement.Item2.x >= size.x || 
                queueElement.Item2.y >= size.y || 
                queueElement.Item2.z >= size.z || 
                queueElement.Item2.x < 0 || 
                queueElement.Item2.y < 0 || 
                queueElement.Item2.z < 0
            ) continue;
            ...
            entropySortedQueue.Add(queueElementSocket, queueElementSocket.Entropy());
        }
    }
    ...
    {
        if(entropySortedQueue.Count == 0) return false;
        ModuleSocket lowestEntropySocket = entropySortedQueue.ExtractMin();
        ...
        {
            GameObject parentGO = spawnSlots[
                lowestEntropySocket.Position.x, 
                lowestEntropySocket.Position.y, 
                lowestEntropySocket.Position.z
            ];
            GameObject collapsedModuleSocket = Instantiate(
                lowestEntropySocket.CollapsedModule.Prefab, 
                parentGO.transform, 
                false
            );
            collapsedModuleSocket.transform.localPosition = Vector3.zero;
            collapsedModuleSocket.transform.rotation = Quaternion.Euler(
                0, 
                90 * lowestEntropySocket.CollapsedModule.Rotation, 
                0
            );
            ...
            for(int y = 0; y < size.y; y++)
            {
                for(int x = 0; x < size.x; x++)
                {
                    if(x == 0 || z == 0 || x == boundary.Size.x-1 || z == boundary.Size.z-1)
                    {
                        ...
                        entropySortedQueue.Add(socket, socket.Entropy());
                    }
                }
            }
        }
